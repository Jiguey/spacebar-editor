<script lang="ts">
  ...
  import { formatTok, formatTokRate, formatDuration } from "$lib/chat/chatFormat";
  import {
...
>
  ...
  <ChatContextFooter
    ...
    {footerUsageView}
    footerUsageTitle={footerUsageTitle()}
    monthlyUsageLabel={monthlyUsageLabel()}
    lastReplyFooterLabel={lastReplyFooterLabel()}
    bd={contextBreakdown()}
    ...
    {contextBarColors}
    contextBudgetOptions={contextBudgetOptions()}
    {compactButtonInactive}
    {compactButtonTitle}
    chatBackend={$settings.chatBackend}
    bind:budgetMenuOpen={contextBudgetMenuOpen}
    onToggleUsageView={() => void toggleFooterUsageView()}
    {getSectionFilePath}
    {getActivePromptFiles}
    onOpenSection={(fp) => void openSectionFile(fp)}
    onOpenBuiltinSection={openBuiltinSection}
    onOpenChatHistory={openChatHistory}
    onToggleBudgetMenu={toggleContextBudgetMenu}
    onPickBudget={pickContextBudgetOption}
    onRequestCompaction={() => requestManualCompaction()}
  />
  {/if}
</div>
